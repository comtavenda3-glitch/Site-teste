...
export function Profile({ user, onLogout, onDeposit, onConfirmDeposit, onWithdraw }: ProfileProps) {
    ...
    return (
      <div className="animate-slide-up">
        ...
        <button
          ...
          disabled={!withdrawCheck.allowed}
  ...
  const renderDepositsHistory = () => (
    <div className="animate-slide-up">
      ...
      <h2 className="text-xl font-bold text-white mb-6">Histórico de Depósitos</h2>

      {user.deposits.length === 0 ? (
        <div className="text-center py-8 bg-dark-700/80 backdrop-blur-sm border border-dark-600 rounded-xl">
          <History className="w-12 h-12 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-400">Nenhum depósito realizado</p>
        </div>
      ) : (
        <div className="space-y-2">
          {user.deposits.map((d, index) => (
            <div 
              key={d.id} 
            ...
            >
              <div>
                <p className="text-white font-medium">R$ {d.amount.toFixed(2)}</p>
                <p className="text-gray-500 text-xs">{new Date(d.createdAt).toLocaleString('pt-BR')}</p>
              </div>
              <span className={`text-xs px-3 py-1 rounded-full ${
                d.status === 'confirmed' ? 'bg-primary-500/20 text-primary-400' : 'bg-yellow-500/20 text-yellow-400'
              }`}>
                {d.status === 'confirmed' ? 'Confirmado' : 'Pendente'}
              </span>
            </div>
          ))}
        </div>
  ...
  const renderWithdrawalsHistory = () => (
    <div className="animate-slide-up">
      ...
      <h2 className="text-xl font-bold text-white mb-6">Histórico de Saques</h2>

      {user.withdrawals.length === 0 ? (
        <div className="text-center py-8 bg-dark-700/80 backdrop-blur-sm border border-dark-600 rounded-xl">
          <History className="w-12 h-12 text-gray-600 mx-auto mb-3" />
          <p className="text-gray-400">Nenhum saque realizado</p>
        </div>
      ) : (
        <div className="space-y-2">
          {user.withdrawals.map((w, index) => (
            <div 
              key={w.id} 
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div>
                  ...
                  <p className="text-gray-500 text-xs">{new Date(w.createdAt).toLocaleString('pt-BR')}</p>
                </div>
                <span className={`text-xs px-3 py-1 rounded-full ${
                  w.status === 'completed' ? 'bg-primary-500/20 text-primary-400' :
              ...
              <div className="text-xs text-gray-500 border-t border-dark-600 pt-2 mt-2">
                <div className="flex justify-between">
                  <span>Taxa:</span>
                  <span className="text-red-400">- R$ {w.fee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Valor líquido:</span>
                  <span className="text-primary-400">R$ {w.netAmount.toFixed(2)}</span>
                </div>
              </div>
            </div>
          ))}
